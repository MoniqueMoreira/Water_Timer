...
int main() {
    ...
    xTaskCreate(__Init_Task__, "Init", 2*1024, NULL, 2, &xTask_Init);
...
static void __Init_Task__(void *params) {
    ...
    if(FLASH_Init() != RETURN_STATUS_OK) {
        LOGGER_Info("SYSTEM", "Erro ao iniciar flash!");
        while (1) vTaskDelay(pdMS_TO_TICKS(1000));
    }
    if (DEVICE_SETTINGS_Init() != RETURN_STATUS_OK) {
        LOGGER_Info("SYSTEM", "Erro ao iniciar Device Settings!");
        while (1) vTaskDelay(pdMS_TO_TICKS(1000));
    }
